    ...
    for (let rowNumber = 1; rowNumber <= maxHeaderRow; rowNumber++) {
      ...
      for (let col = 1; col <= maxColumn; col++) {
        ...
        for (const [key, aliases] of Object.entries(fieldAliases)) {
          if (Number.isInteger(map[key])) continue;
          if (aliases.some(alias => headerMatches(normalized, alias))) map[key] = col;
        }
      }
      const requiredHits = ['courseId','schoolYear','title','level'].filter(k => Number.isInteger(map[k])).length;
      const optionalHits = Object.keys(map).filter(k => k !== '__headerRow' && Number.isInteger(map[k])).length;
      const score = requiredHits * 1000 + optionalHits;
      if (score > bestScore) { best = map; bestScore = score; }
      if (requiredHits === 4) break;
    }

    const headerRow = best.__headerRow || 1;
    let plausibleRows = 0;
    const sampleEnd = Math.min(worksheet.rowCount || 1, headerRow + 25);
    for (let r = headerRow + 1; r <= sampleEnd; r++) {
      const row = worksheet.getRow(r);
      if (cellText(row.getCell(1)).trim() || cellText(row.getCell(5)).trim()) plausibleRows++;
    }
    if ((worksheet.columnCount || 0) >= 6 && plausibleRows > 0) {
      const known = {
        courseId: 1, schoolYear: 2, category: 3, subject: 4, title: 5,
        level: 6, description: 8, schedule: 9, startDate: 11, endDate: 12,
        totalDuration: 13, places: 15, additionalInfo: 17,
        locationName: 19, locationRoom: 20, trainer: 35
      };
      for (const [key, col] of Object.entries(known)) {
        if (!Number.isInteger(best[key])) best[key] = col;
      }
    }
    return best;
  }
